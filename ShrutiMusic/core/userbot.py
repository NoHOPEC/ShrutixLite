    ...
    async def start(self):
        ...
        if config.STRING1:
            try:
                ...
                # Check if LOG_GROUP_ID is valid
                if not config.LOG_GROUP_ID:
                    LOGGER(__name__).error("LOG_GROUP_ID is not set in config!")
                    exit()
                
                LOGGER(__name__).info(f"Trying to send message to LOG_GROUP_ID: {config.LOG_GROUP_ID}")
                
                # Try to send message to logger group with better error handling
                try:
                    # First check if we can get chat info
                    chat_info = await self.one.get_chat(config.LOG_GROUP_ID)
                    LOGGER(__name__).info(f"Logger group info: {chat_info.title}")
                    
                    # Check if assistant is member of the group
                    member = await self.one.get_chat_member(config.LOG_GROUP_ID, self.one.id)
