...
# MongoDB collection for VC logger settings
vcloggerdb = mongodb.vclogger

prefixes = [".", "!", "/", "@", "?", "'"]

# Function to load VC logger status from database
async def load_vc_logger_status():
    """Load VC logger status from MongoDB on startup"""
    try:
        ...
        # Start monitoring for all enabled chats
...
# Function to save VC logger status to database
async def save_vc_logger_status(chat_id: int, status: bool):
    """Save VC logger status to MongoDB"""
    try:
...
# Function to get VC logger status (from memory first, then database)
async def get_vc_logger_status(chat_id: int) -> bool:
    """Get VC logger status for a chat"""
...
async def monitor_vc_chat(chat_id):
    ...
    while chat_id in active_vc_chats and await get_vc_logger_status(chat_id):
        ...
        await asyncio.sleep(5)  # Real-time feel without hitting API limits
...
async def check_and_monitor_vc(chat_id):
    ...
    if not userbot:
        return
    try:
        # Always start monitoring if enabled, regardless of current participants
...
# Initialize VC logger on bot startup
async def initialize_vc_logger():
    """Call this function when bot starts to load saved settings"""
